...
@womens_stores = Store.where(mens_apparel: false, womens_apparel: true, annual_revenue: { less_than: 1000000 })

@womens_stores.each do |store|
  puts "#{store.name} has an annual revenue or #{store.annual_revenue}"
end
